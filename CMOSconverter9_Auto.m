function cmosData = CMOSconverter9_Auto(fdim1,fdim2,sigma,ngauss,scale,upperframelimit,inputfreq,drawbounds)
...
%cmosData = CMOSconverter9_Auto(3,3,0.5,2,1,1e3,50,0);

%cmosData = CMOSconverter9_Auto(3,3,0.5,2,1,1e3,100,0);

%cmosData = CMOSconverter9_Auto(3,3,0.5,2,1,1500,250,0);
%cmosData = CMOSconverter9_Auto(3,3,0.5,2,1,3e3,500,0);
%cmosData = CMOSconverter9_Auto(3,3,0.5,2,1,5e3,1000,0);
...
while count <= total
    ...
    if PlaneCount > 1
        ...
        omeXML = omeXML(1:1150);
        k=strfind(omeXML, "ExposureTime=");
        
        strfind(omeXML, "http://www.openmicroscopy.org/Schemas/OME/2016-06");
        
        
        xpotime = omeXML(k+14:end);
        quoteend = find(xpotime == '"',1,'first');
        xpotime = str2double(xpotime(1:quoteend-1));
        
        if xpotime ~= 0 && ~isnan(xpotime)
            if contains(omeXML, "http://www.openmicroscopy.org/Schemas/OME/2016-06")
                frequency = 1/xpotime; %Aquisition Frequency
            elseif contains(omeXML, "http://www.openmicroscopy.org/Schemas/OME/2015-01")
                frequency = 1000/xpotime; %Aquisition Frequency
            else
                error('Please check bioformats OME schema version')
            end
        ...
        acqFreq = xpotime; % Acquisition time in msec
